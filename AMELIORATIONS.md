...
>
> **Chantier §5.2 réalisé (15/07/2026) :** tests golden croisés des deux générateurs PDF (`src/lib/__tests__/pdf-golden.test.ts`) — la même convocation est générée par le front et par l'edge, le texte rendu est extrait et les champs critiques (candidat, session, nom/SIRET/NDA du centre) vérifiés identiques. Verdict initial : générateurs cohérents. En-têtes d'avertissement posés dans les deux fichiers. La fusion reste hors périmètre (volontairement).
>
> **Chantier §5.1 réalisé (15/07/2026) :** les états métier des notes `[AUTO]` sont désormais structurés — colonnes `auto_category`/`auto_metadata` sur `contact_historique` (migration Lovable `20260715095753`), écriture unique via `createAutoNote`, lectures à catégorie prioritaire avec repli titre pour l'historique, 7 tests unitaires. Le string-matching disparaîtra de lui-même à mesure que les notes anciennes sortent des fenêtres de lecture (90 j pour les reports).
>
> **Élagage (15/07/2026, décisions directeur sur inventaire d'usage réel) :** supprimés côté écrans (tables conservées) — **LMS** (0 inscription : admin, quiz du portail candidat, hooks, ~5 400 lignes) et **Planning conduite + réservation publique** (0 séance/réservation : ~3 700 lignes). Gardés : Produits/forfaits, Objectifs, Cartes pro, Partenaires (décision directeur) ; **Qualiopi et Enquêtes satisfaction à activer, pas à supprimer** (obligations réglementaires). **Reporté : Docs pédagogiques** — table vide mais code entrelacé avec le portail candidat, les sessions et Qualiopi (6 fichiers sensibles) ; à traiter avec le chantier documentaire.
>
