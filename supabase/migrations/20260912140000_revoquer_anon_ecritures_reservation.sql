-- ════════════════════════════════════════════════════════════════════════════
-- Fermeture de l'accès ANONYME aux 2 fonctions d'ÉCRITURE du flux de réservation.
-- ════════════════════════════════════════════════════════════════════════════
--
...
--
-- ── Constat ──────────────────────────────────────────────────────────────────
-- reserver_creneau_public(text, uuid) et annuler_reservation_public(text, uuid, text)
-- sont SECURITY DEFINER (elles contournent la RLS) et ÉCRIVENT :
...
--   • Son jumeau de LECTURE validate_reservation_token(text) a déjà été révoqué
--     pour anon (20260911190000 : « flux de réservation non câblé »).
--   • La policy de lecture « Anon view available creneaux » a été supprimée dès
--     20260710104031. Il ne reste donc QUE ces deux grants d'ÉCRITURE, résiduels.
...
-- REVOKE EXECUTE ... FROM PUBLIC, anon sur les deux fonctions.
...
-- anon_execute doit repasser à false sur les deux ; authenticated inchangé.
