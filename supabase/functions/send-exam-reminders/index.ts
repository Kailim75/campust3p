...
serve(async (req) => {
  ...
  try {
    ...
    const { data: examensT3PJ7, error: examensT3PError } = await supabase
      .from("examens_t3p")
      .select(`
        id, date_examen, heure_examen, centre_examen,
        type_formation, numero_tentative,
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin: 16px 0;">
                <tr>
                  <td style="background-color: #faf5ff; border-left: 4px solid #7c3aed; border-radius: 6px; padding: 18px 20px;">
                    ...
                    <p style="margin: 0; font-size: 13px; color: #555;"><strong>🎯 Tentative :</strong> ${examen.numero_tentative || 1}/3</p>
                  </td>
                </tr>
              </table>
              <h3 style="margin: 18px 0 10px 0; color: #333; font-size: 15px;">Documents obligatoires :</h3>
              <ul style="margin: 0; padding-left: 20px;">
                <li style="margin-bottom: 4px;">✅ Pièce d'identité en cours de validité</li>
                <li style="margin-bottom: 4px;">✅ Convocation à l'examen</li>
                <li style="margin-bottom: 4px;">✅ Attestation de formation</li>
              </ul>
              <p style="margin: 16px 0 0 0; color: #7c3aed; font-weight: bold;">Bon courage pour votre examen !</p>
            `,
          });
    ...
    const { data: examensPratiquesJ1, error: examensPratiquesError } = await supabase
      ...
      .select(`
        id, date_examen, heure_examen, lieu_examen,
        type_examen, numero_tentative,
        contact:contacts(id, nom, prenom, email)
      `)
    ...
    if (examensPratiquesError) {
      ...
      for (const examen of examensPratiquesJ1) {
        ...
        try {
          const htmlContent = buildEmailHtml({
            title: "🚗 Examen Pratique — C'est demain !",
            accentColor: "#059669",
            recipientName: `${contact.prenom} ${contact.nom}`,
            bodyHtml: `
              <p style="margin: 0 0 12px 0;"><strong>Votre examen pratique est prévu demain !</strong></p>
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin: 16px 0;">
                <tr>
                  <td style="background-color: #ecfdf5; border-left: 4px solid #059669; border-radius: 6px; padding: 18px 20px;">
                    <p style="margin: 0 0 6px 0; font-weight: 700; color: #047857;">Examen Pratique — ${examen.type_examen}</p>
                    <p style="margin: 0 0 4px 0; font-size: 13px; color: #555;"><strong>📅 Date :</strong> ${formatDateFr(examen.date_examen)}</p>
                    ${examen.heure_examen ? `<p style="margin: 0 0 4px 0; font-size: 13px; color: #555;"><strong>⏰ Heure :</strong> ${examen.heure_examen}</p>` : ""}
                    ${examen.lieu_examen ? `<p style="margin: 0 0 4px 0; font-size: 13px; color: #555;"><strong>📍 Lieu :</strong> ${examen.lieu_examen}</p>` : ""}
                    <p style="margin: 0; font-size: 13px; color: #555;"><strong>🎯 Tentative :</strong> ${examen.numero_tentative || 1}/3</p>
                  </td>
                </tr>
              </table>
              <h3 style="margin: 18px 0 10px 0; color: #333; font-size: 15px;">Éléments à prévoir :</h3>
              <ul style="margin: 0; padding-left: 20px;">
                <li style="margin-bottom: 4px;">✅ Véhicule conforme aux normes</li>
                <li style="margin-bottom: 4px;">✅ Pièce d'identité en cours de validité</li>
                <li style="margin-bottom: 4px;">✅ Permis de conduire</li>
                <li style="margin-bottom: 4px;">✅ Attestation de formation</li>
                <li style="margin-bottom: 4px;">✅ Convocation à l'examen</li>
              </ul>
              <p style="margin: 16px 0 0 0; color: #059669; font-weight: bold;">Présentez-vous 15 minutes avant l'heure prévue. Bonne chance !</p>
            `,
          });
