...
serve(async (req) => {
  ...
  try {
    ...
    if (!to || !subject || !messageBody || !centreId) {
      throw new Error("to, subject, body, and centreId are required");
    }
    ...
    // Get active email account for this centre
    ...
    if (accErr || !account) throw new Error("No active email account for this centre");
    ...
    if (threadErr) throw new Error("Failed to create CRM thread: " + threadErr.message);
    ...
    if (linkEntity?.entityType && linkEntity?.entityId) {
      await supabase.from("crm_email_links").insert({
        ...
        is_primary: true,
        link_source: "manual",
        linked_by: authUser?.user?.id || null,
      });
    }
