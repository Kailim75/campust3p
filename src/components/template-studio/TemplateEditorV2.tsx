// ═══════════════════════════════════════════════════════════════
// TemplateEditorV2 — Template editor using v2 hooks
// Replaces TemplateEditorTab.tsx (v1) — Phase 7A
...
import {
  ...
  useCreateTemplateV2,
  useUpdateTemplateV2,
  usePublishTemplateV2,
  useArchiveTemplateV2,
  type TemplateV2,
...
export default function TemplateEditorV2({ templateId, isCreating, onBack, onGenerate, aiPrefilledBody, aiPrefilledType }: Props) {
  ...
  // v2 workflow: draft → published (compliance gate handles validation)
  const handlePublish = async () => {
    if (!template) return;
    setIsPublishing(true);
    try {
      // Build a template object with current editor state for compliance gate
      const templateToPublish: TemplateV2 = {
        ...template,
        template_body: body,
        name,
        description: description || null,
  ...
  // v2 workflow: can publish from draft (compliance gate is the guard)
  const canPublish = template && (template.status === "draft" || template.status === "approved");
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center text-muted-foreground">
          ...
          <p className="text-sm">Ou créez-en un nouveau depuis la bibliothèque</p>
        </CardContent>
      </Card>
    );
  }
