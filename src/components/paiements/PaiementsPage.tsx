...
export function PaiementsPage() {
  ...
  const handleExportComptable = async (exportFormat: "csv" | "xlsx") => {
    ...
    const exportData = filteredFactures.map((facture) => ({
      ...
      "Client": facture.contact ? `${facture.contact.prenom} ${facture.contact.nom}` : ((facture as any).client_partner?.company_name || ""),
      "Email": facture.contact?.email || (facture as any).client_partner?.email || "",
      "Téléphone": facture.contact?.telephone || (facture as any).client_partner?.phone || "",
  ...
  const sendRelance = useCallback(async (facture: FactureWithDetails) => {
    ...
    try {
      await envoyerRelancePaiement({
        ...
        montantRestant: Number(facture.montant_total) - facture.total_paye,
      ...
      toast.error("Erreur lors de l'envoi de la relance: " + ((err as Error).message || "Erreur inconnue"), { id: "relance-" + facture.id });
    }
  }, []);
