...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"
  ...
  <w:body>
    ...
    <w:sectPr>
      ...
      <w:pgMar w:top="400" w:right="500" w:bottom="400" w:left="500" w:header="400" w:footer="400" w:gutter="0"/>
    </w:sectPr>
  </w:body>
</w:document>`;
}
...
function buildBody(
  ...
  // ── Title ──
  xml += centeredParagraph("FEUILLE D'ÉMARGEMENT", 24, T3P.forestGreen, true, 80, 20);
  xml += centeredParagraph(session.nom, 18, T3P.warmGray700, false, 0, 40);

  // ── Compact session info ──
  xml += sessionInfoTable(session);

  // ── Combined emargement table: all dates as columns ──
  xml += combinedEmargementTable(sessionDates, contacts, emargements);
...
function headerBand(session: SessionInfo): string {
  ...
  return singleCellTable(T3P.forestGreen, tableWidth, [
    { text: esc(session.centre_nom || "ÉCOLE T3P"), size: 26, color: T3P.white, bold: true },
    ...(subLine ? [{ text: esc(subLine), size: 14, color: T3P.creamLight }] : []),
    ...(idLine ? [{ text: esc(idLine), size: 12, color: T3P.creamLight }] : []),
  ]);
}
...
function sessionInfoTable(session: SessionInfo): string {
  const cols = [
    ...
    { label: "Période", value: session.date_debut && session.date_fin ? `${format(new Date(session.date_debut), "dd/MM/yyyy")} au ${format(new Date(session.date_fin), "dd/MM/yyyy")}` : "—" },
  ];
  const colW = Math.floor(15838 / cols.length);

  let rows = "<w:tr>";
  cols.forEach(({ label, value }) => {
    rows += `<w:tc><w:tcPr><w:tcW w:w="${colW}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${T3P.cream}"/><w:tcMar><w:top w:w="40" w:type="dxa"/><w:bottom w:w="40" w:type="dxa"/><w:left w:w="80" w:type="dxa"/></w:tcMar></w:tcPr>
      <w:p><w:pPr><w:spacing w:after="0"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="14"/><w:color w:val="${T3P.forestGreen}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t>${esc(label)}</w:t></w:r></w:p>
      <w:p><w:pPr><w:spacing w:after="0"/></w:pPr><w:r><w:rPr><w:sz w:val="14"/><w:color w:val="${T3P.warmGray700}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t xml:space="preserve">${esc(value)}</w:t></w:r></w:p>
    </w:tc>`;
  });
  rows += "</w:tr>";
  ...
  <w:tbl>
    <w:tblPr><w:tblW w:w="15838" w:type="dxa"/><w:jc w:val="center"/>
      <w:tblBorders>
  ...
  <w:p><w:pPr><w:spacing w:before="40" w:after="0"/></w:pPr></w:p>`;
}
function combinedEmargementTable(
  sessionDates: Date[],
  contacts: { id: string; nom: string; prenom: string }[],
  emargements: EmargementData[]
): string {
  const totalWidth = 15838; // landscape usable width
  const nameColWidth = Math.min(3500, Math.floor(totalWidth * 0.22));
  const dateColWidth = Math.floor((totalWidth - nameColWidth) / sessionDates.length);

  // Grid columns
  const gridCols = `<w:gridCol w:w="${nameColWidth}"/>${sessionDates.map(() => `<w:gridCol w:w="${dateColWidth}"/>`).join("")}`;

  // Header row with dates
  let headerRow = `<w:tr><w:trPr><w:trHeight w:val="400" w:hRule="atLeast"/></w:trPr>`;
  headerRow += `<w:tc><w:tcPr><w:tcW w:w="${nameColWidth}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${T3P.gold}"/><w:vAlign w:val="center"/><w:tcMar><w:left w:w="60" w:type="dxa"/></w:tcMar></w:tcPr>
    <w:p><w:pPr><w:jc w:val="center"/><w:spacing w:after="0"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="16"/><w:color w:val="${T3P.forestGreen}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t>Nom et Prénom</w:t></w:r></w:p>
  </w:tc>`;

  sessionDates.forEach((date) => {
    const dayLabel = format(date, "EEE", { locale: fr });
    const dateLabel = format(date, "dd/MM", { locale: fr });
    headerRow += `<w:tc><w:tcPr><w:tcW w:w="${dateColWidth}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${T3P.gold}"/><w:vAlign w:val="center"/></w:tcPr>
      <w:p><w:pPr><w:jc w:val="center"/><w:spacing w:after="0"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="14"/><w:color w:val="${T3P.forestGreen}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t xml:space="preserve">${esc(dayLabel)}</w:t></w:r></w:p>
      <w:p><w:pPr><w:jc w:val="center"/><w:spacing w:after="0"/></w:pPr><w:r><w:rPr><w:sz w:val="14"/><w:color w:val="${T3P.forestGreen}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t>${esc(dateLabel)}</w:t></w:r></w:p>
    </w:tc>`;
  });
  headerRow += `</w:tr>`;
  ...
  contacts.forEach((contact, idx) => {
    ...
    contactRows += `<w:tr><w:trPr><w:trHeight w:val="500" w:hRule="atLeast"/></w:trPr>`;
    contactRows += `<w:tc><w:tcPr><w:tcW w:w="${nameColWidth}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${bg}"/><w:vAlign w:val="center"/><w:tcMar><w:left w:w="60" w:type="dxa"/></w:tcMar></w:tcPr>
      <w:p><w:pPr><w:spacing w:after="0"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="16"/><w:color w:val="${T3P.warmGray700}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t xml:space="preserve">${esc(contact.nom.toUpperCase())} ${esc(contact.prenom)}</w:t></w:r></w:p>
    </w:tc>`;

    sessionDates.forEach((date) => {
      const dateStr = format(date, "yyyy-MM-dd");
      const dateEmargements = emargements.filter(e => e.date_emargement === dateStr && e.contact_id === contact.id);
      const morning = dateEmargements.find(e => e.periode === "matin");
      const afternoon = dateEmargements.find(e => e.periode === "apres_midi");
      const best = (morning?.signature_url || morning?.present) ? morning : afternoon;
      const status = sigStatus(best);

      const rpr = status.color
        ? `<w:sz w:val="14"/><w:color w:val="${status.color}"/><w:b/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/>`
        : `<w:sz w:val="14"/><w:color w:val="${T3P.warmGray500}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/>`;

      contactRows += `<w:tc><w:tcPr><w:tcW w:w="${dateColWidth}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${bg}"/><w:vAlign w:val="center"/></w:tcPr>
  ...
  for (let i = 0; i < 2; i++) {
    ...
    emptyRows += `<w:tr><w:trPr><w:trHeight w:val="500" w:hRule="atLeast"/></w:trPr>`;
    emptyRows += `<w:tc><w:tcPr><w:tcW w:w="${nameColWidth}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${bg}"/></w:tcPr><w:p><w:pPr><w:spacing w:after="0"/></w:pPr></w:p></w:tc>`;
    sessionDates.forEach(() => {
      emptyRows += `<w:tc><w:tcPr><w:tcW w:w="${dateColWidth}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${bg}"/></w:tcPr><w:p><w:pPr><w:spacing w:after="0"/></w:pPr></w:p></w:tc>`;
    });
    emptyRows += `</w:tr>`;
  }
...
function signatureBlocks(): string {
  ...
  <w:tbl>
    ...
    <w:tr>
      <w:tc><w:tcPr><w:tcW w:w="${colW}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${T3P.cream}"/><w:tcMar><w:top w:w="40" w:type="dxa"/><w:bottom w:w="40" w:type="dxa"/><w:left w:w="120" w:type="dxa"/></w:tcMar></w:tcPr>
        <w:p><w:r><w:rPr><w:b/><w:sz w:val="16"/><w:color w:val="${T3P.forestGreen}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t>Signature du formateur :</w:t></w:r></w:p>
        <w:p/><w:p/>
      </w:tc>
      <w:tc><w:tcPr><w:tcW w:w="${colW}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="${T3P.cream}"/><w:tcMar><w:top w:w="40" w:type="dxa"/><w:bottom w:w="40" w:type="dxa"/><w:right w:w="120" w:type="dxa"/></w:tcMar></w:tcPr>
        <w:p><w:pPr><w:jc w:val="right"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="16"/><w:color w:val="${T3P.forestGreen}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t>Cachet de l'organisme :</w:t></w:r></w:p>
        <w:p/><w:p/>
      </w:tc>
    </w:tr>
  </w:tbl>`;
