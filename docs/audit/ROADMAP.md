...
**Reste à faire (vague 2)** : fil d’Ariane visible, brancher SoftDeleteConfirmDialog sur les suppressions à impact, garde anti-perte de saisie (brouillon), vue mobile de la session, accessibilité (aria-label, sélecteurs clavier), action groupée de statut, window.confirm restants (SessionParcoursTab, FormationsPage, DuplicatesDialog). **Perf** : framer-motion hors chemin critique. **Vague 3a (avant le 2ᵉ centre)** : inchangée, et confirmée par le scan Lovable (LMS, budget prévisionnel, tables financières encore sans colonne `centre_id`). **À activer** : `CRON_SECRET` (procédure corrigée dans supabase/CRON_JOBS.md — ajouter l’en-tête aux jobs AVANT de créer le secret).

**Points mineurs relevés par les relecteurs, non traités** (aucun n’est bloquant) : validation client du montant d’un versement (champ sans `min`, aller-retour serveur pour un montant ≤ 0) ; `SignaturesTrackingPanel` désactive le bouton d’envoi de toutes les lignes pendant un envoi ; liste des apprenants encore sur l’agrégat là où la fiche est passée au calcul par facture ; conventions de statuts de facture divergentes entre écrans (brouillon/annulée) ; `activeTab` d’`AutomationsPage` non réinitialisé après un deep-link d’onboarding.
